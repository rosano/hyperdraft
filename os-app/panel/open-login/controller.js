...
exports.WKCLoginSubmitAction = function(req, res, next) {
	...
	return req.OLSKSharedConnectionFor('KVCSharedConnectionMongo').OLSKConnectionClient.db(process.env.WKC_SHARED_DATABASE_NAME).collection('wkc_members').find({}).toArray(function(err, items) {
		if (err) {
		...
		return res.redirect(req.body.WKCLoginReturnPath ? decodeURIComponent(req.body.WKCLoginReturnPath) : res.locals.OLSKCanonicalFor('KVCWriteRoute'));
	});
};
