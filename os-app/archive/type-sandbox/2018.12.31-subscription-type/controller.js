...
exports.WKCSandboxSubscriptionTypeAction = function(req, res, next) {
	return req.OLSKSharedConnectionFor('KVCSharedConnectionMongo').OLSKConnectionClient.db(process.env.WKC_SHARED_DATABASE_NAME).collection('wkc_subscriptions').find({
		...
		return Promise.all(items.slice(-1).map(function (e) {
			return new Promise(function (resolve, reject) {
				return req.OLSKSharedConnectionFor('KVCSharedConnectionMongo').OLSKConnectionClient.db(process.env.WKC_SHARED_DATABASE_NAME).collection('wkc_subscriptions').findOneAndUpdate({
