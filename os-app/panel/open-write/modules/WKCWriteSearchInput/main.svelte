<script>
export let SearchInputText = '';
export let SearchInputPlaceholder = '';
export let OLSKLocalized = null;

import { createEventDispatcher } from 'svelte';
const dispatch = createEventDispatcher();
function SearchInputDispatchClear() {
	dispatch('SearchInputDispatchClear');
}
</script>

<div class="WKCWriteSearchInputContainer OLSKToolbarFlexible">

<input bind:value={ SearchInputText } placeholder={ SearchInputPlaceholder || OLSKLocalized('WKCWriteSearchInputPlaceholderText') } accesskey="f" class="WKCWriteSearchInput" autofocus />

{#if SearchInputText}
	<button on:click={ SearchInputDispatchClear } class="WKCWriteSearchInputClearButton OLSKLayoutElementTappable OLSKLayoutButtonNoStyle" style="background-image: url('/panel/_shared/ui-assets/wIKFilterClear.svg')" title={ OLSKLocalized('WKCWriteSearchInputClearButtonText') }></button>
{/if}

</div>

<style>
.WKCWriteSearchInputContainer {
	/* @ContainerStation:Parent */
	position: relative;

	/* @ContainerFlexbox:Parent */
	display: flex;

	/* ChromeRemoveInputOutline */
	outline: none;
}

input {
	padding: 4px;
	margin: 0;
	border: 1px solid #e6e6e6;
	border-radius: 5px;

	text-indent: 2px;
	line-height: 1.4;

	/* @ContainerFlexbox:Child */
	flex-grow: 1;
}

.WKCWriteSearchInputClearButton {
	--WKCWriteSearchInputClearButtonSize: 24px;

	width: var(--WKCWriteSearchInputClearButtonSize);
	height: var(--WKCWriteSearchInputClearButtonSize);

	background-size: cover;

	/* @ContainerStation:Child */
	position: absolute;
	right: 4px;
	top: 50%;
	margin-top: calc(-1 * var(--WKCWriteSearchInputClearButtonSize) / 2);
}

@media screen and (max-width: 760px) {

input {
	height: 30px;

	font-size: 14px;

	/* MobileSafariRemoveDefaultInputStyle */
	-webkit-appearance: none;
	-moz-appearance: none;
	appearance: none;
}

}

</style>
